...
PANDA_CORE_API void scale(Polygon& poly, float scale);
PANDA_CORE_API Polygon scaled(const Polygon& poly, float scale);

/// Rotate every point of the paths around the center, angle is given in radians
PANDA_CORE_API void rotate(Polygon& poly, const Point& center, float angle);
PANDA_CORE_API Polygon rotated(const Polygon& poly, const Point& center, float angle);
